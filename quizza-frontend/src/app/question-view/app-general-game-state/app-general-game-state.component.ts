    ...
    <div class="game-state-container">
      ...
      <ng-container *ngIf="this.gameStateService.gameState$ | async as gameState; else loading">
        ...
        <div class="section">
          ...
          <p><strong>Round:</strong> {{ gameState.currentRound }} / {{ gameState.maxRounds }}</p>
          <p><strong>Round Time Limit:</strong> {{ gameState.maxRoundTime }}s</p>
          <p><strong>Current Round Time Remaining:</strong> {{ gameState.roundTime }}s</p>
        </div>

        <!-- Current Question -->
        <div class="section" *ngIf="gameState.currentQuestion">
            <h3>Current Question (ID: {{ gameState.currentQuestion.id }})</h3>
            <p><strong>Category:</strong> {{ gameState.currentQuestion.category }}</p>
            <p><strong>Question:</strong> {{ gameState.currentQuestion.question }}</p>
        ...
        <div class="section" *ngIf="gameState.preGameState">
            <h3>Pre-Game Lobby</h3>
             ...
             <ul class="scrollable-list small-list">
                <li *ngFor="let name of gameState.preGameState.playerNames">{{ name }}</li>
             </ul>
             <h4>Player Votes (Vote Start?):</h4>
             <ul class="scrollable-list small-list">
                <!-- Use keyvalue pipe to iterate over the map -->
                <li *ngFor="let voteEntry of gameState.preGameState.playerVotes | keyvalue">
                   {{ voteEntry.value.playerName }} (ID: {{ voteEntry.key }}): {{ voteEntry.value.voteStart }}
                </li>
             </ul>
        </div>
          ...
          <div class="scrollable-list player-state-list">
            ...
            <div *ngFor="let playerEntry of gameState.playerSpecificGameState | keyvalue" class="player-state-item">
              ...
              <ul *ngIf="playerEntry.value.allAnswers.size > 0; else noAnswersPlayer">
                ...
                <li *ngFor="let answerEntry of playerEntry.value.allAnswers | keyvalue">
              ...
              <ng-template #noAnswersPlayer><p>No answers recorded yet.</p></ng-template>
            </div>
          </div>
        </div>
